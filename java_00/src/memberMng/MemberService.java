...
public class MemberService {
	...
	// 변경하고자 하는 곳 선택 cntl + F -> Replace 바꿀 단어 쓰고 replace/Find 누르면 변경 
	...
	public void updateMember() {
		// 							DAO에 있음(findAllMember)
		List<Member> members = mDAO.findMember();

		System.out.print("수정 할 회원 아이디를 입력해 주세요. : ");
	...
	public void printAllMembers() {
		...
		for(int i=0; i<members.size(); i++) {
			...
			displayMsg("회원 비밀번호 : " + member.getMemberPw() );
			displayMsg("회원 이름 : " + member.getMemberName() );
			displayMsg("회원 아메일 : " + member.getEmail() );
			displayMsg("회원 연락처 : " + member.getPhone() );
			displayMsg( "---------------------------------");
